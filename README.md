...
- 공통된 하단 네비게이션바를 적용하여 화면의 일반화 및 유저의 일관적이고 차별화된 경험을 제공
...
- NestedScrollView를 사용하여 좌우 스크롤을 적용하여 한정된 화면에 다양한 정보를 표시할 수 있음.
- 또한 사용자의 자연스런 제스처 사용유도가 가능.
...
구현 방식 및 특징
- onTabSelectedListener를 통해 하단 네비게이션 바를 공통적으로 표시되도록 하였으며, <br>
  애니메이션을 통해 현재 어느 탭에 머물고 있는지 표시되도록 하였음
- 하단 네비게이션 바에는 맞춤 꽃 추천 서비스와 원데이 클래스 신청 기능, 커뮤니티 기능 등<br>
  우리 서비스의 핵심 기능으로 이동하는 버튼을 구현하여였으며, 이를 통해 보다 쉬운 서비스 이용이 되도록 하였음.
